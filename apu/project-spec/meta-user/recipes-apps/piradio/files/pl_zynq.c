...
static volatile uint8_t *axisswitch_base;
...
int pl_init(void)
{
    ...
    axisswitch_base = mmap(NULL, AXI_AXISSWITCH_ADDR_RANGE,
                    PROT_READ | PROT_WRITE,
                    MAP_SHARED,
                    mem_fd, AXI_AXISSWITCH_BASE_ADDR);

    if (axisswitch_base == MAP_FAILED) {
...
void pl_deinit(void)
{
    ...
    if (axisswitch_base)
        munmap((void *)axisswitch_base, AXI_AXISSWITCH_ADDR_RANGE);        
    if (mem_fd >= 0)
        close(mem_fd);
}
...
{
    return *(volatile uint32_t *)(axisswitch_base + offset);
}

void pl_axisswitch_write32(uint32_t offset, uint32_t val)
{
    *(volatile uint32_t *)(axisswitch_base + offset) = val;
}
...
void pl_axisswitch_reset() {
    ...
    // disable all routes
    for(int i = 0; i < 14; i++) {
        pl_axisswitch_write32(AXISSWITCH_MUX_OFFSET + 4*i, wdata);
    }
}
